...
class TestColonist:
    ...
    def test_decide_fallback(self):
        ...
        c = Colonist(COLONIST_TEMPLATES[0], random.Random(42))
        vm = Lispy(seed=42)
        # With no context vars, personality may fail — should fallback
        action = c.decide({}, vm)
...
class TestColony:
    ...
    def test_relationships_initialized(self):
        ...
        for c in colony.colonists:
            assert len(c.relationships) == 9  # everyone except self
...
class TestMars100:
    ...
    def test_resources_change(self):
        ...
        # At least one resource should change
    ...
    def test_different_seeds_diverge(self):
        ...
        # Populations should differ at some point
...
class TestSubSimulations:
    def test_sub_sim_archive_populated(self):
        ...
        sim = Mars100(seed=42)
        results = sim.run(years=50)
        # May or may not have sub-sims depending on seed
...
class TestPopulationDynamics:
    def test_deaths_possible(self):
        ...
        # With all resources at 0, deaths are very likely
        # But not guaranteed in a single year
    ...
    def test_births_after_year_10(self):
        """Births should not occur before year 10."""
        sim = Mars100(seed=42)
        for year in range(1, 10):
            result = sim.tick(year)
            assert result["births"] == []

    def test_births_possible_after_year_10(self):
        """Births can occur after year 10."""
        sim = Mars100(seed=42)
        any_births = False
        for year in range(1, 100):
            ...
            if result["births"]:
                any_births = True
                break
        # Not guaranteed but likely over 90 years
...
class TestActions:
    ...
    def test_all_personality_actions_valid(self):
        ...
        import re
        for tmpl in COLONIST_TEMPLATES:
            # Extract quoted symbols from personality
