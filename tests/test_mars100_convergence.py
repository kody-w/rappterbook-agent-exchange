...
class TestValueConvergence:
    ...
    def test_identical_stats_yield_zero_convergence(self):
        colonists = create_founding_ten(42)
        # Force all stats to the same value
        for c in colonists:
...
class TestConvergenceTrend:
    """Tests for the overall convergence trend in SimulationResult."""
    ...
    def test_trend_in_serialized_output(self):
        ...
        assert d["summary"]["convergence_trend"] in ("converging", "diverging", "stable")
...
class TestDeepSubSimExpressions:
    """Tests that depth-2/3 sub-sims use governance-themed expressions."""
    ...
    def test_deep_governance_expr_depth_2(self):
        ...
        # Should reference governance-related concepts
    ...
    def test_deep_governance_expr_depth_3(self):
        ...
        assert "(let" in expr
        # Depth-3 expressions should reference recursion depth or meta concepts
    ...
    def test_subsim_log_appears_in_year(self):
        ...
        assert subsim_count >= 0  # May be 0 due to randomness, but shouldn't crash
...
class TestInsightExtraction:
    """Tests for the insight queue and promotion pipeline."""

    def test_insight_queue_accumulates(self):
        engine = Mars100Engine(seed=42, total_years=30)
        engine.run()
        # With 30 years, we should have some insights (from deep sub-sims)
        # This may be 0 if randomness doesn't trigger deep sims — that's OK
    ...
    def test_promoted_insights_list(self):
        ...
        assert isinstance(engine.promoted_insights, list)
...
class TestDraftAmendment:
    """Test the _draft_amendment static method."""
...
class TestSerialization:
    """Verify new fields survive serialization."""
    ...
    def test_simulation_result_promoted_insights_serializes(self):
        ...
        assert isinstance(d["summary"]["promoted_insights"], list)
...
class TestLethalityRegression:
    """Ensure the colony isn't trivially wiped out."""

    def test_100_year_survival(self):
        engine = Mars100Engine(seed=42, total_years=100)
        result = engine.run()
        # At least 50 years should be simulated (colony doesn't die by year 50)
    ...
    def test_multiple_seeds_not_all_extinct(self):
        ...
        for seed in range(42, 52):
            ...
            if len(result.years) >= 80:
                survivors += 1
        # At least 5 of 10 seeds should survive 80+ years
        assert survivors >= 5, (
