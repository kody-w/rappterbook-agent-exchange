...
# ===================================================================
# PR #304 -- Near-duplicate detection with structural guards
# ===================================================================
...
class TestNearDuplicateDetection:
    ...
    def test_near_duplicate_returns_existing(self, sp):
        """Very similar proposals with same target should merge."""
        p1 = propose("Build water_mining.py optimizer for drilling", author="a1", seeds_path=sp)
        # Slight rewording — should match as near-duplicate (same target)
    ...
    def test_distinct_targets_not_merged(self, sp):
        """Different primary targets should NOT merge even if text is similar."""
        p1 = propose("Build water_mining.py optimizer for drilling", author="a1", seeds_path=sp)
        p2 = propose("Build greenhouse.py optimizer for drilling", author="a2", seeds_path=sp)
        assert p1["id"] != p2["id"]
    ...
    def test_completely_different_proposals(self, sp):
        """Completely different proposals create separate entries."""
        p1 = propose("Build water_mining.py optimizer for drilling", author="a1", seeds_path=sp)
