...
class TestOrbitalGeometry:
    ...
    def test_closest_approach(self):
        ...
        assert 60e6 < closest < 100e6  # circular orbit model
    ...
    def test_light_delay_at_1au(self):
        ...
        assert 490 < delay < 510  # ~499 seconds

    def test_solar_exclusion_at_conjunction(self):
        assert lc.is_solar_exclusion(0)

    def test_no_exclusion_at_opposition(self):
        assert not lc.is_solar_exclusion(int(lc.SYNODIC_PERIOD_SOLS / 2))
...
class TestAtmosphericLoss:
    ...
    def test_transmission_zero_for_zero_tau(self):
...
class TestPhotonCounting:
    def test_photon_rate_positive_at_opposition(self):
        d = 78.3e6
        rate = lc.received_photons_per_second(d)
        assert rate > 0
...
class TestTick:
    ...
    def test_blocked_sols_counted(self, terminal):
        lc.tick(terminal, sol=0)
    ...
    def test_peak_throughput_tracked(self, terminal):
        lc.tick(terminal, sol=int(lc.SYNODIC_PERIOD_SOLS / 2), dust_tau=0.3)
...
class TestSmokeFullYear:
    ...
    def test_full_year_terabytes_transferred(self):
        results = lc.run_simulation(sols=668, dust_tau=0.5)
        total_tb = results[-1].total_data_gb / 1000.0
        assert total_tb > 0.1  # at least 100 GB over a year
