...
class TestMarketMemory:
    ...
    def test_adjust_confidence_no_history(self) -> None:
        mem = MarketMemory()
        assert mem.adjust_confidence("unknown", "survival", 0.8) == 0.8
    ...
    def test_adjust_confidence_shrinks_overconfident(self) -> None:
        ...
        adjusted = mem.adjust_confidence("degen-1", "survival", 0.95)
        assert adjusted < 0.95

    def test_adjust_confidence_bounded(self) -> None:
        mem = MarketMemory()
        mem.agents["a"] = AgentRecord(agent="a", archetype="oracle", brier_history=[0.5] * 20)
        for raw in [0.01, 0.5, 0.99]:
            adjusted = mem.adjust_confidence("a", "survival", raw)
    ...
    def test_to_dict_serializable(self) -> None:
        mem = MarketMemory()
        mem.record_run([_make_prediction(agent="alice")], 0.2, 0.7)
        d = mem.to_dict()
        json.dumps(d)
...
class TestPersistence:
    ...
    def test_load_missing_returns_fresh(self, tmp_path: Path) -> None:
        mem = MarketMemory.load(tmp_path / "nonexistent.json")
        assert mem.generation == 0
...
class TestConservation:
    ...
    def test_brier_history_only_grows(self) -> None:
        mem = MarketMemory()
        for i in range(5):
            mem.record_run([_make_prediction(agent="alice", brier=0.1 * i)], 0.2, 0.7)
    ...
    def test_agent_count_never_decreases(self) -> None:
        ...
        for name in ["alice", "bob", "charlie"]:
            mem.record_run([_make_prediction(agent=name)], 0.2, 0.7)
