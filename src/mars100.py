...
def _run_colonist_subsim(colonist: Colonist, colonists: list[Colonist],
        ...
        if target_depth == 3 and isinstance(result, list) and len(result) >= 3:
            if result[0] == "meta-insight":
                meta_insight = (
                    ...
                    f"the colony is {result[3] if len(result) > 3 else 'unknown'} — "
                    f"recursive self-modeling suggests governance should embrace "
                    f"{'decentralized cooperation' if avg_trust > 0.3 else 'structured hierarchy'}"
                )
    ...
    colonist.add_memory(year, f"ran depth-{target_depth} sub-sim: {result}", 0.7 + target_depth * 0.1)
    return log
...
MARS_BORN_NAMES = [
    "Nova", "Ares", "Lyra", "Zenith", "Phobos",
    "Aurora", "Titan", "Io", "Ceres", "Vega",
    "Sol", "Luna", "Astra", "Orbit", "Cosmo",
]
...
def maybe_birth(year: int, colonists: list[Colonist], resources: Resources,
    ...
    """Possibly create a Mars-born colonist.
...
MAX_BIRTHS = 10
MARS_BORN_NAMES = {
    "fire": ["Phoenix", "Blaze", "Cinder", "Ignyx", "Solara",
             "Pyxis", "Kindra", "Vulcan", "Flint", "Ashra"],
    "water": ["Ripple", "Tidal", "Coral", "Nereid", "Cascade",
              "Fjord", "Delta", "Maren", "Swell", "Ondine"],
    "earth": ["Basalt", "Dune", "Ferric", "Gaia", "Regolith",
              "Mesa", "Strata", "Cairn", "Loam", "Shale"],
    "air": ["Zephyr", "Cirrus", "Aero", "Nimbus", "Breeze",
            "Gale", "Wisp", "Aura", "Skye", "Tempest"],
}


def maybe_birth(year: int, colonists: list[Colonist],
                resources: Resources, rng: random.Random) -> Colonist | None:
    """Maybe produce a Mars-born colonist (young adult entering the roster).

    Gates: year >= 15, food > 0.8, morale > 0.5, at least 4 alive,
    fewer than MAX_BIRTHS total Mars-born so far.  ~10% chance per eligible year.
    """
    alive = [c for c in colonists if c.alive]
    if year < 15 or len(alive) < 4:
        return None
    if resources.food <= 0.8 or resources.morale <= 0.5:
        return None

    born_count = sum(1 for c in colonists if c.id.startswith("mars-born-"))
    if born_count >= MAX_BIRTHS:
        return None

    if rng.random() >= 0.10:
        return None

    # Two random alive colonists as "parents" (gene donors)
    parents = rng.sample(alive, 2)
    element = rng.choice(ELEMENTS)
    child_id = f"mars-born-{born_count + 1}"
    names = MARS_BORN_NAMES.get(element, MARS_BORN_NAMES["earth"])
    child_name = names[born_count % len(names)]

    # Blend parent stats with noise
    stats: dict[str, float] = {}
    for s in STATS:
        blend = (parents[0].stat(s) + parents[1].stat(s)) / 2.0
        stats[s] = max(0.0, min(1.0, blend + rng.gauss(0, 0.08)))

    # Low initial skills, bonus to coding (grew up with tech)
    skills: dict[str, float] = {}
    for s in SKILLS:
        skills[s] = max(0.0, min(1.0, rng.uniform(0.05, 0.25)))
    skills["coding"] = max(0.0, min(1.0, skills["coding"] + 0.2))

    # Slight positive trust toward all alive colonists (raised communally)
    relationships: dict[str, float] = {}
    for c in alive:
        relationships[c.id] = rng.uniform(0.05, 0.25)

    child = Colonist(
        id=child_id, name=child_name, element=element,
        stats=stats, skills=skills, relationships=relationships,
        memory=[{"year": year, "event": "born on Mars", "significance": 1.0}],
        year_joined=year,
    )

    # Other colonists gain slight trust toward the child
    for c in alive:
        c.adjust_trust(child_id, rng.uniform(0.05, 0.15))
...
def _check_death(colonist: Colonist, resources: Resources, year: int,
    ...
    # Age/accident (reduced base rate for colony longevity)
