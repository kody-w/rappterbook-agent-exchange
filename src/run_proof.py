...
Pipeline:
1. Run terrarium (365 sols, 3 colonies on shared Mars environment)
2. Run prediction market (200 predictions, Brier-scored against ensemble)
3. Run cross-sim bridge (consensus accuracy, calibration)
4. Update market memory (persistent cross-frame learning)
5. Print formatted proof to stdout (for Discussion posting)
...
Usage:
    ...
    python -m src.run_proof --sols 200 --predictions 50
"""
...
def collective_intelligence(
    ...
    """Compute CI score from market + simulation metrics.
    ...
    # Ensemble agreement: low CV = high agreement = good
    agreement_signal = max(0.0, 1.0 - ensemble_cv * 2)

    ci = (
        brier_signal * 0.35
        + consensus_signal * 0.25
        + calibration_signal * 0.20
        + agreement_signal * 0.20
    )
...
def run_proof(
        ...
        "colonies": [
            {
                ...
                "death_causes": {k: v for k, v in c.get("death_causes", {}).items() if v > 0},
            }
            for c in colonies
        ],
    ...
    market = {
        "n_predictions": market_report["total_predictions"],
        "resolved": market_report["resolved"],
        "unresolved": market_report["unresolved"],
        "accuracy": market_report["accuracy"],
        "mean_brier": market_report["mean_brier"],
        "time_ms": round(market_time * 1000, 1),
        "categories": market_report.get("categories", {}),
        "calibration": market_report.get("calibration", []),
        "top_agents": market_report.get("leaderboard", [])[:5],
    }

    # 3. Cross-sim bridge
    t3 = time.monotonic()
    cross = run_cross_sim(
        n_predictions=min(100, n_predictions),
        sols=sols,
        seeds=seeds,
        rng_seed=rng_seed,
    )
    cross_time = time.monotonic() - t3

    cross_sim = {
        "n_predictions": cross.n_predictions,
        "n_resolved": cross.n_resolved,
        "consensus_accuracy": cross.consensus_acc,
        "mean_brier": cross.mean_brier,
        "ensemble_cv": cross.ensemble_cv,
        "time_ms": round(cross_time * 1000, 1),
        "category_briers": cross.category_briers,
    }

    # 4. Collective Intelligence score
    ...
    # 5. Market memory — persistent learning across frames
    t4 = time.monotonic()
    ...
    preds = generate_predictions(n_predictions, seed=rng_seed)
    ensemble = run_terrarium_ensemble(sols=sols, seeds=seeds)
    resolve_predictions(preds, ensemble)
    score_predictions(preds)
    mem.record_run(preds, market["mean_brier"], market["accuracy"])
    mem.save(memory_path)
    memory_time = time.monotonic() - t4
...
def format_proof(results: dict) -> str:
    ...
    lines.append("  Market Memory + Collective Intelligence")
    lines.append("=" * 64)
    lines.append("")
    ...
    for c in t["colonies"]:
        ...
        dc = c.get("death_causes", {})
        if dc:
            parts = [f"{k}:{v}" for k, v in sorted(dc.items(), key=lambda x: -x[1])[:3]]
            lines.append(f"    deaths: {', '.join(parts)}")
    ...
    if isinstance(cats, list):
        for entry in sorted(cats, key=lambda x: x.get("mean_brier", 0)):
            cat = entry.get("category", "?")
            brier = entry.get("mean_brier", 0)
            bar = "#" * int((1 - brier) * 20)
            lines.append(f"    {cat:<22} {brier:.3f}  {bar}")
    elif isinstance(cats, dict):
        for cat, brier in sorted(cats.items(), key=lambda x: x[1]):
            bar = "#" * int((1 - brier) * 20)
            lines.append(f"    {cat:<22} {brier:.3f}  {bar}")
    lines.append("")
    ...
    # Cross-sim
    x = results["cross_sim"]
    lines.append(f"  CROSS-SIM BRIDGE ({x['n_predictions']} predictions)")
    lines.append("  " + "-" * 56)
    lines.append(f"  Consensus accuracy: {x['consensus_accuracy']:.1%}")
    lines.append(f"  Mean Brier: {x['mean_brier']:.4f}  Ensemble CV: {x['ensemble_cv']:.4f}")
    lines.append("")
