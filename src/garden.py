...
Writes docs/garden_state.json for the visualization. Python stdlib only.
"""
...
def tick(s):
    ...
    for sp in nsp-osp: s["events"].append({"e":ep,"t":"speciation","m":f"Species '{sp}' emerged (pop: {s['species'][sp]['count']})"})
    for sp in osp-nsp: s["events"].append({"e":ep,"t":"extinction","m":f"Species '{sp}' went extinct"})
    ...
    if ep%50==0 and s["species"]:
        ...
        s["events"].append({"e":ep,"t":"milestone","m":f"Epoch {ep}: {len(s['organisms'])} org, {len(s['species'])} species. Dominant: {dom}"})
    return s
...
def main():
    ...
    if s is None:
        ...
        print(f"Epoch {s['epoch']} -> {s['epoch']+1} | Pop: {len(s['organisms'])}")
        s=tick(s)
    save(s)
    print(f"Done! Epoch {s['epoch']}: {len(s['organisms'])} organisms, {len(s.get('species',{}))} species")
    for ev in [e for e in s.get("events",[]) if e["e"]==s["epoch"]]:
        print(f"  [{ev['t'][0].upper()}] {ev['m']}")
