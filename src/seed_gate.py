...
Evolution log:
    ...
    PR #304  -- graduated multi-target scoring (2→+1.0, 3→+1.5,
                  ...
                  (Jaccard 3-gram shingles with article-stripping
                  normalization); negation-aware tag/question
                  fallback; BatchResult.summary(); wired similarity
                  into propose_seed.py for near-duplicate detection.
"""
...
def _normalize_for_similarity(text: str) -> str:
    ...
    Lowercase, strip articles (the/a/an) when followed by another word,
    collapse whitespace, strip conventional commit prefixes.
    Keeps underscores, dots, and hyphens so filenames retain structure.
    """
...
def _trigram_shingles(text: str) -> set[str]:
    """Return the set of character 3-gram shingles from normalized *text*."""
...
def validate_seed(
    ...
    *,
    ...
    #    text expresses negative intent (e.g. "Don't build anything")
    ...
    # Also detect contraction negations: "don't" → tokens "don" + "t"
    _has_contraction_neg = bool(_lower_words & _CONTRACTION_NEG_STEMS)
    _has_negation_word = False
    if _raw_neg_words or _has_contraction_neg:
        _has_negation_word = True
        # Affirmative exceptions: "not only", "not just"
        if _raw_neg_words == {"not"} and not _has_contraction_neg and re.search(r"\bnot\s+(?:only|just)\b", _lower_text):
            _has_negation_word = False
        # Question stem exceptions: "why not" is a proposal, not negation
        if _raw_neg_words <= {"not", "nor"} and not _has_contraction_neg and _QUESTION_STEM_RE.match(_lower_text.strip()):
            _has_negation_word = False
