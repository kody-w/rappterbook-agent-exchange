...
_CONTRACTION_NEG_STEMS: frozenset[str] = frozenset({
    ...
    "couldn", "can", "isn", "aren", "wasn", "weren", "hasn",
    "haven", "hadn", "ain",
})
...
def _cli() -> None:
    """CLI entry point: validate, --explain, --batch."""
    ...
    if "--batch" in args:
        ...
        results = [validate(p) for p in proposals]
    ...
    if "--explain" in args:
        ...
        if not args:
            print("Usage: seed_gate.py --explain <proposal>", file=sys.stderr)
            sys.exit(2)
        text = " ".join(args)
        result = explain_dict(text)
        print(_json.dumps(result, indent=2))
        sys.exit(0)

    if not args:
        print("Usage: seed_gate.py [--explain] [--batch] <proposal>", file=sys.stderr)
        sys.exit(1)
