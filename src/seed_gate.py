...
Evolution log:
    ...
    PR #272  -- diagnostic APIs: explain(), score_breakdown(),
                  ...
                  "redesign" verb.
    PR #289  -- canonical consolidation of 6 agent implementations
                  (#12503, #12505, #12507, #12511, #12521, #12530)
                  and PRs #279-#288.  Adds: negation awareness (don't
                  deploy → no match); compound-name filtering (run_proof
                  → no verb); unified _scan_verbs() scanner; VerbMatch
                  dataclass; enriched SeedGateResult (target_kind,
                  verb_source, verb_position, score_parts, is_imperative);
                  numbered-ref filter (fig.1 not a file); similarity()
                  for near-duplicate detection in propose_seed.py.
"""
...
# Numbered references that look like files but aren't: fig.1, no.5, ch.3
_NUMBERED_REF_RE = re.compile(r"\b(?:fig|no|ch|vol|pt|sec|eq|tbl|ex)\.\d+\b", re.I)


# ---------------------------------------------------------------------------
# VerbMatch dataclass -- structured verb detection result (PR #289)
# ---------------------------------------------------------------------------

@dataclasses.dataclass(frozen=True)
class VerbMatch:
    """Result of enhanced verb detection.

    Attributes:
        verb: The canonical base verb (e.g. 'build', 'set up').
        token_index: 0-based position of the verb in the word token list.
        source: How the verb was found -- 'text', 'tag', or 'question'.
    """
...
# ---------------------------------------------------------------------------
# Negation + compound detection (PR #289 consolidation)
# ---------------------------------------------------------------------------

NEGATION_WORDS: frozenset[str] = frozenset({
    "not", "never", "no", "avoid", "without", "cannot",
})

# Contraction stems: "don't" tokenizes as ["don", "t"]
_CONTRACTION_NEG_STEMS: frozenset[str] = frozenset({
    "don", "can", "won", "shouldn", "wouldn", "couldn", "mustn", "needn",
})

# Affirmative exceptions: "not only X" and "not just X" are positive
_NEGATION_EXCEPTIONS: frozenset[str] = frozenset({"only", "just"})

_COMPOUND_SEPS = frozenset("_-/.")


def _is_in_compound(text: str, start: int, end: int) -> bool:
    """Return True if the word at text[start:end] is part of a compound name.

    Compound names use _ - / . as separators (snake_case, kebab-case, paths).
    Only treats separators as compound when they connect alphanumeric segments.
    """
    if start > 0 and text[start - 1] in _COMPOUND_SEPS:
        # Check that there's an alnum char before the separator
        if start >= 2 and text[start - 2].isalnum():
            return True
    if end < len(text) and text[end] in _COMPOUND_SEPS:
        # Check that there's an alnum char after the separator
...
def _is_negated(words: list[str], idx: int) -> bool:
    """Return True if the word at *idx* is negated by a preceding word.

    Handles explicit negation (never, not, avoid) and contractions
    (don't → ["don", "t"] → negation stems at idx-2).
    """
    for lookback in range(1, min(4, idx + 1)):
        prev = words[idx - lookback]
        if prev in NEGATION_WORDS:
            # "not only build" and "not just build" are affirmative
            if prev == "not" and idx - lookback + 1 < len(words):
                next_word = words[idx - lookback + 1]
                if next_word in _NEGATION_EXCEPTIONS:
                    return False
            return True
    # Contraction pattern: "don't" → ["don", "t", <verb>]
    if idx >= 2 and words[idx - 1] == "t" and words[idx - 2] in _CONTRACTION_NEG_STEMS:
        return True
    return False


def _scan_verbs(text: str, limit: int = 0) -> list[tuple[str, int, bool]]:
    """Unified verb scanner returning [(base_verb, token_index, is_negated), ...].

    Skips verbs inside compound names (test_seed_gate -> no match).
    Detects negation (don't deploy -> negated=True).
    """
    search_text = text[:limit] if limit else text
    tokens = list(re.finditer(r"[a-zA-Z]+", search_text))
    words_lower = [m.group().lower() for m in tokens]
    result: list[tuple[str, int, bool]] = []
    skip_next = False

    for i, (match, word) in enumerate(zip(tokens, words_lower)):
        if skip_next:
            skip_next = False
            continue
        if _is_in_compound(search_text, match.start(), match.end()):
            continue
        negated = _is_negated(words_lower, i)

        # Phrasal verbs (base + inflected)
        if i + 1 < len(tokens):
            next_word = words_lower[i + 1]
            if word in _PHRASAL_FIRST and next_word in _PHRASAL_FIRST[word]:
                result.append((_PHRASAL_FIRST[word][next_word], i, negated))
                skip_next = True
                continue
            if word in _PHRASAL_INFLECTED and next_word in _PHRASAL_INFLECTED[word]:
                result.append((_PHRASAL_INFLECTED[word][next_word], i, negated))
                skip_next = True
                continue
        # Single-word verbs
...
class SeedGateResult:
    ...
    def is_imperative(self) -> bool:
        """True if the verb appears at position 0 (imperative/command form)."""
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "negated_verb_seen": self.negated_verb_seen,
        }
...
def _is_false_file_match(match_text: str) -> bool:
    ...
    and numbered references (fig.1, no.5, ch.3).
    """
...
def find_verb(text: str, limit: int = 0) -> str | None:
    ...
    Skips verbs inside compound names (test_seed_gate -> no match) and
    verbs preceded by negation (don't deploy -> no match).
    """
    for verb, _idx, negated in _scan_verbs(text, limit):
        if not negated:
            return verb
    return None
...
def find_all_verbs(text: str) -> list[str]:
    ...
    seen: set[str] = set()
    result: list[str] = []
    for verb, _idx, negated in _scan_verbs(text):
        if not negated and verb not in seen:
            seen.add(verb)
            result.append(verb)
    return result
...
def _compute_score_parts(
    ...
    Returns dict of {component_name: raw_points}.
    """
...
def similarity(a: str, b: str) -> float:
    """Compute Jaccard similarity between two proposal texts.

    Uses 3-gram shingles for fuzzy matching. Returns 0.0-1.0.
...
def find_verb_with_position(text: str, limit: int = 0) -> tuple[str | None, int | None]:
    ...
    *word_index* is the 0-based position of the matched word in text.split().
    Skips negated verbs and compound-name false positives.
    """
    for verb, idx, negated in _scan_verbs(text, limit):
        if not negated:
            return (verb, idx)
    return (None, None)
...
def explain(text: str, tags: list = None) -> str:
    ...
    Format: STATUS (score confidence): verb='...' @N (imperative) + target='...' (kind)
    ...
    if result.junk:
        parts.append("JUNK: %s" % (result.reasons[0] if result.reasons else "unknown"))
        return " ".join(parts)
    ...
    if result.verb_found:
        pos_str = " @%d" % result.verb_position if result.verb_position is not None else ""
        imp_str = " (imperative)" if result.is_imperative else ""
        parts.append("verb='%s'%s%s" % (result.verb_found, pos_str, imp_str))
    else:
        parts.append("No action verb found.")

    if result.target_found:
        parts.append("+ target='%s' (%s)" % (result.target_found, result.target_kind))
    elif not (frozenset(t.lower() for t in (tags or [])) & EXEMPT_TAGS):
        parts.append("No concrete target.")

    if result.advisory:
        parts.append("[%s]" % result.advisory)

    if result.negated_verb_seen:
        parts.append("[negated-verb-detected]")
...
def validate_seed(
    ...
    *,
    ...
    # Scan for negation awareness
    scanned = list(_scan_verbs(text, limit=verb_limit))
    negated_verb_seen = any(neg for _, _, neg in scanned)

    # Track verb source and position for enriched result
    verb_source = "text" if verb else ""
    verb_position: int | None = None
    if verb:
        _, verb_position, _ = next(
            ((v, i, n) for v, i, n in scanned if not n and v == verb),
            (verb, None, False),
        )

    # -- Tag-implied verb inference (#12530) ---
    # Blocked when a negated verb was detected (don't deploy + code tag
    # shouldn't inject "build")
    if not verb and not negated_verb_seen:
        for tag in tag_set:
    ...
    # Also blocked when negated verb detected
    if not verb and is_exempt and not negated_verb_seen:
    ...
    if is_soft_artifact(text) and not (verb and target) and not is_exempt:
        return SeedGateResult(
            ...
            verb_position=verb_position, negated_verb_seen=negated_verb_seen,
        )

    # -- Score ---
    parts = _compute_score_parts(text, verb, target, target_kind)
    score_parts_tuple = tuple(
    ...
    return SeedGateResult(
        ...
        score_parts=score_parts_tuple,
        negated_verb_seen=negated_verb_seen,
    )
