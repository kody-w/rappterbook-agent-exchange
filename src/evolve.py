...
Each run = one tick of evolution. Organisms with DNA-encoded traits
compete, reproduce with mutation, and die. Species emerge and diverge.
State persists in state/world.json. Visualization reads docs/state.json.
...
STATE_PATH = REPO_ROOT / "state" / "world.json"
OUTPUT_PATH = REPO_ROOT / "docs" / "state.json"

WORLD_W, WORLD_H = 800, 600
MAX_POP, INIT_POP = 300, 50
RES_SPAWN, MAX_RES, RES_ENERGY = 25, 300, 22.0
MAX_AGE = 180
...
def spname(sid):
    ...
    ps = [_SYL[int(h[i:i+2],16) % len(_SYL)] for i in range(0,6,2)]
    return "{}{}{}us".format(ps[0].capitalize(), ps[1], ps[2])

def mkorg(x, y, dna, sid, pid=None, gen=0):
    return {"id":nid(),"dna":dna,"x":round(x,1),"y":round(y,1),
        "vx":0.0,"vy":0.0,"energy":100.0,"age":0,"generation":gen,
        "parent":pid,"species_id":sid,"cooldown":0,"traits":dtraits(dna),"kills":0}

def empty():
    return {"_meta":{"tick":0,"epoch":"Primordial Soup","created_at":niso(),"updated_at":niso(),
        "total_births":0,"total_deaths":0,"total_species":0},
        "config":{"world_w":WORLD_W,"world_h":WORLD_H},
...
def seed(w):
    ...
    for i in range(nf):
        ...
        for _ in range(ct):
            dna = mutdna(bd, 0.25)
            org = mkorg(random.uniform(20,WORLD_W-20), random.uniform(20,WORLD_H-20), dna, sid)
...
def td(x1,y1,x2,y2):
    dx=min(abs(x1-x2),WORLD_W-abs(x1-x2)); dy=min(abs(y1-y2),WORLD_H-abs(y1-y2))
    return math.sqrt(dx*dx+dy*dy)

def mvto(o,tx,ty,s):
    dx,dy=tx-o["x"],ty-o["y"]
    if abs(dx)>WORLD_W/2: dx=-(dx/abs(dx))*(WORLD_W-abs(dx))
    if abs(dy)>WORLD_H/2: dy=-(dy/abs(dy))*(WORLD_H-abs(dy))
    d=math.sqrt(dx*dx+dy*dy)
    if d<0.1: return
    f=min(s,d)/d; o["vx"]=round(dx*f,2); o["vy"]=round(dy*f,2)
    o["x"]=round(wrap(o["x"]+o["vx"],WORLD_W),1); o["y"]=round(wrap(o["y"]+o["vy"],WORLD_H),1)
...
def tick(w):
    ...
    for _ in range(min(RES_SPAWN, MAX_RES-len(res))):
        res.append({"x":round(random.uniform(5,WORLD_W-5),1),"y":round(random.uniform(5,WORLD_H-5),1),
            "energy":round(random.uniform(8,RES_ENERGY),1)})
    ...
    for o in orgs:
        ...
        if o["energy"]>t["repro_threshold"] and o["cooldown"]<=0 and len(orgs)+len(births)<MAX_POP:
            ...
            child=mkorg(wrap(o["x"]+math.cos(a)*10,WORLD_W),wrap(o["y"]+math.sin(a)*10,WORLD_H),cd,si,o["id"],o["generation"]+1)
    ...
    for o in orgs:
        if o["id"] in deaths:
            ...
            w["graveyard"].append({"id":o["id"],"species":o["species_id"],"generation":o["generation"],"age":o["age"],"kills":o["kills"],"tick":tn})
    ...
    for sid,sd in w["species"].items():
        ...
        if p==0 and sd.get("first_seen",0)<tn and "extinct_tick" not in sd:
            sd["extinct_tick"]=tn; evts.append({"tick":tn,"type":"extinction","desc":"{} went extinct".format(sd["name"])})
    if not w["organisms"]:
...
def save(w):
    ...
    with open(OUTPUT_PATH,"w") as f: json.dump(viz,f,indent=1)
