...
class EnsembleResult:
    ...
    # Per-colony, per-sol percentile bands: bands[colony_idx][metric][percentile_idx][sol]
    bands: list[dict[str, list[list[float]]]]
    # Per-colony aggregate stats
    final_pop_stats: list[dict[str, float]]
    growth_pct_stats: list[dict[str, float]]
    survival_rates: list[float]
    # Best single-seed run (canonical for event annotations)
...
def run_ensemble(
    ...
    Each run uses env_seed = base_seed + i, keeping colony seeds fixed
    (so colony RNG varies with env, testing robustness to environment).
    """
    ...
    # Collect per-seed, per-colony, per-sol values
    # all_series[colony_idx][metric][seed_idx] = list[float] of length sols
    ...
    canonical_seed = base_seed + 42  # middle-ish seed
    ...
    for i in range(n_seeds):
        ...
        for ci, col_data in enumerate(results["colonies"]):
            ...
            for m in metrics:
                ...
                # Pad if shorter than sols (colony died)
    ...
    for ci in range(n_colonies):
        ...
        for m in metrics:
            # For each percentile, compute per-sol value
    ...
    return EnsembleResult(
        ...
        colony_strategies=colony_strategies,
        bands=bands,
        final_pop_stats=final_pop_stats,
        growth_pct_stats=growth_pct_stats,
