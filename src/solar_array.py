...
# --- Physical constants ---
PANEL_EFFICIENCY_BASE = 0.30          # GaAs triple-junction at 25°C
TEMP_COEFF_PER_C = -0.002            # efficiency loss per °C below 25°C
TEMP_REF_C = 25.0                     # reference temperature for panel rating
MIN_TEMP_DERATING = 0.60              # floor: panels still work at -120°C
...
def temperature_derating(temp_c: float) -> float:
    ...
    Loss = 0.2% per °C below the 25°C reference temperature.
    """
    cold_penalty = abs(TEMP_COEFF_PER_C) * max(0.0, TEMP_REF_C - temp_c)
    return max(MIN_TEMP_DERATING, 1.0 - cold_penalty)
...
def solar_power_sol(
    ...
    Power = effective_area × flux × cell_efficiency × temp_derating × sol_hours / 1000
    """
    if solar_flux_wm2 <= 0:
        return 0.0
...
def degrade_panels(array: SolarArray, radiation_msv: float) -> float:
    ...
    """
    rad_factor = max(1.0, radiation_msv / 0.67)  # normalized to GCR baseline
...
def tick_power_system(
    ...
    # Meet demand from generation, then battery
