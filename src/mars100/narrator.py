...
def narrate_year(year_result: dict, rng: random.Random) -> str:
    ...
    traditions = year_result.get("traditions_accepted", [])
    if traditions:
        lines.append(f"\n### Oral Traditions (+{len(traditions)})")
        for t in traditions[:3]:
            lines.append(f"- [{t.get('category', '?')}] \"{t.get('text', '?')[:60]}\" "
                         f"(trust {t.get('trust_rating', 0):.0%})")
...
def generate_final_report(sim_result: dict) -> str:
    ...
    oral = sim_result.get("oral_history", {})
    all_traditions = oral.get("traditions", [])
    active_traditions = [t for t in all_traditions if not t.get("archived")]
    archived_traditions = [t for t in all_traditions if t.get("archived")]
    if active_traditions or archived_traditions:
        lines.extend(["## Oral Tradition", "",
                       f"- **Active traditions:** {len(active_traditions)}",
                       f"- **Archived traditions:** {len(archived_traditions)}",
                       ""])
        if active_traditions:
            by_cat: dict[str, int] = {}
            for t in active_traditions:
                cat = t.get("category", "unknown")
                by_cat[cat] = by_cat.get(cat, 0) + 1
            lines.append("**Active by category:** " +
                          ", ".join(f"{c}({n})" for c, n in sorted(by_cat.items())))
            lines.append("")
            lines.append("**Top traditions:**")
            top = sorted(active_traditions, key=lambda t: -t.get("trust_rating", 0))[:5]
            for t in top:
                lines.append(f"- [{t.get('category')}] \"{t.get('text', '?')[:70]}\" "
                              f"— trust {t.get('trust_rating', 0):.0%}, cited {t.get('citations', 0)}x")
            lines.append("")
