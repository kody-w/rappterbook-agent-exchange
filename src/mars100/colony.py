...
def tick_resources(resources: Resources, active_count: int,
                   ...
                   infra_modifiers: dict[str, float] | None = None,
                   ecology_modifiers: dict[str, float] | None = None) -> dict[str, float]:
    ...
    eco = ecology_modifiers or {}
    for name in RESOURCE_NAMES:
        current = getattr(resources, name)
        prod_mult = eco.get(f"{name}_production_mult", 1.0)
