...
from src.mars100.economics import (
    ...
    endow_child as econ_endow_child,
...
class Mars100Engine:
    ...
    def _check_births(self) -> list[dict]:
        ...
        for parent_a, parent_b, _ in pairs[:1]:
            if self.rng.random() < birth_prob:
                ...
                endowment = econ_endow_child(child, parent_a, parent_b)
    ...
    def tick(self) -> YearResult:
        ...
        # --- economic governance proposals (dedicated RNG stream) ---
        econ_proposal_log: dict | None = None
        if active and should_propose_tax_change(self.economics, self.year, self.econ_rng):
            proposer = self.econ_rng.choice(active)
            econ_proposal_log = generate_economic_proposal(
                self.economics, self.governance.gov_type,
                proposer.id, self.year, self.econ_rng)
        ...
        econ_result.economic_proposal = econ_proposal_log
