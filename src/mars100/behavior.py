...
Engine v10.0.  Deterministic — no separate RNG stream needed.
...
# Perturbation caps
STRESS_WEIGHT_CAP: float = 0.3         # max weight shift from stress
MORALE_WEIGHT_CAP: float = 0.2         # max weight shift from morale
PURPOSE_WEIGHT_CAP: float = 0.15       # max weight shift from purpose
LEARNED_WEIGHT_CAP: float = 0.25       # max weight shift from learned prefs

# Social contagion
STRESS_CONTAGION_CAP: float = 0.05     # max contagion per tick
LONELINESS_CONTAGION_CAP: float = 0.03 # smaller to avoid double-count w/ psych
PURPOSE_CONTAGION_CAP: float = 0.04
TRUST_THRESHOLD: float = 0.3           # min trust for contagion to flow

# Learning
LEARNING_RATE: float = 0.05            # per-tick preference adjustment
DECAY_RATE: float = 0.01               # preference decay toward neutral
PREF_CAP: float = 1.0                  # absolute max preference magnitude
SMALL_PREF_THRESHOLD: float = 0.005    # prune prefs below this
...
def compute_action_perturbation(
    ...
    Returns a dict mapping action name → additive weight delta.
    Positive = more likely, negative = less likely.
    All deltas are bounded by their respective caps.
    """
...
def compute_social_contagion(
    ...
    Uses frozen psych snapshot so all colonists see the same pre-contagion
    state (simultaneous update, no order bias).

    trust_pairs: list of (other_id, trust_value) for this colonist.
    """
    ...
    for other_id, trust in trust_pairs:
        ...
        if other is None:
            continue
        w = trust - TRUST_THRESHOLD  # effective weight
...
def update_learned_preferences(
    ...
    """Update learned action preferences based on action outcome.

    Uses action-linked reinforcement: only the resources directly
    associated with the action contribute to learning.
    Returns the updated preference dict.
    """
    profile.total_actions += 1

    # Compute reward signal from action-linked resources
    linked_resources = ACTION_RESOURCE_MAP.get(action_taken, [])
    reward = 0.0
    for res_name in linked_resources:
        reward += resource_delta.get(res_name, 0.0)

    # Update preference for the action taken
    old_pref = profile.action_preferences.get(action_taken, 0.0)
    new_pref = old_pref + LEARNING_RATE * reward
    new_pref = max(-PREF_CAP, min(PREF_CAP, new_pref))
...
def compute_risk_tolerance(
    ...
    """Compute a colonist's risk tolerance from psychological state.

    Returns value in [0, 1]. Higher = more risk-tolerant.
    """
    raw = (RISK_BASE
