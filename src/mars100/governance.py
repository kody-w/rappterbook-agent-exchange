...
No structure is pre-ordained — what emerges depends on the colony's
history, resources, and social dynamics.
"""
...
GOVERNANCE_TYPES = (
    "anarchy", "council", "dictator", "lottery",
    "consensus", "ai_governor",
)
...
class GovernanceProposal:
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            "subsim_result": self.subsim_result,
...
class GovernanceState:
    ...
    leader_id: str = None
    council_ids: list = field(default_factory=list)
    term_end_year: int = None
    ai_program: str = None
    constitution: list = field(default_factory=list)
    history: list = field(default_factory=list)

    def to_dict(self) -> dict:
        """Serialize."""
        return {
            "gov_type": self.gov_type, "leader_id": self.leader_id,
            "council_ids": self.council_ids,
            "term_end_year": self.term_end_year,
            "ai_program": self.ai_program,
    ...
    def from_dict(cls, d: dict) -> "GovernanceState":
        ...
        return cls(
            ...
            council_ids=d.get("council_ids", []),
            term_end_year=d.get("term_end_year"),
            ai_program=d.get("ai_program"),
            constitution=d.get("constitution", []),
            history=d.get("history", []),
        )


def should_propose(year: int, gov_state: GovernanceState,
    ...
    if (gov_state.term_end_year is not None
            and year >= gov_state.term_end_year):
        return True
    return rng.random() < 0.08 + (year / 500)


def generate_proposal(year: int, proposer_id: str,
                      current_gov: GovernanceState,
                      rng: random.Random) -> GovernanceProposal:
    """Generate a governance proposal from a colonist."""
    ...
    if gov_type == "council":
        cs = rng.choice([3, 5])
        ty = rng.choice([5, 10])
        params = {"council_size": cs, "term_years": ty}
    ...
    return GovernanceProposal(
        id=f"prop-y{year}-{gov_type}",
        year=year, proposer_id=proposer_id,
        gov_type=gov_type, parameters=params,
        rationale=rationale,
    )


def resolve_vote(proposal: GovernanceProposal,
...
def apply_governance(proposal: GovernanceProposal,
                     state: GovernanceState,
                     active_ids: list,
                     rng: random.Random) -> None:
    """Apply a passed governance proposal to the colony state."""
    state.history.append({
        "year": proposal.year,
        "from": state.gov_type,
        "to": proposal.gov_type,
    ...
    if proposal.gov_type == "council":
        ...
        state.term_end_year = (proposal.year + term if term > 0
                               else None)
    elif proposal.gov_type == "lottery":
        state.leader_id = rng.choice(active_ids)
        state.term_end_year = (proposal.year
                               + params.get("rotation_years", 1))
    ...
    state.constitution.append(
        f"Year {proposal.year}: governance changed to {proposal.gov_type}")
