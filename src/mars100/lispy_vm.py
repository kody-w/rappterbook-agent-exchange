...
def _parse_atom(token: str) -> Atom:
    ...
    if token.startswith('"') and token.endswith('"'):
        # Keep quotes as marker so evaluator distinguishes strings from symbols
        return token
    try:
        return int(token)
    except ValueError:
...
def _parse_list(tokens: list, pos: int) -> tuple:
...
class Env:
    ...
    def lookup(self, name: str) -> Any:
        """Look up a binding, searching parent scopes."""
...
def evaluate(expr: Expr, env: Env, state: VMState = None,
             depth: int = 0) -> Any:
    ...
    if state.steps > state.max_steps:
        raise LispyBudgetExceeded(
            f"step budget exceeded ({state.max_steps} steps)")
    if depth > state.max_depth:
        raise LispyBudgetExceeded(
            f"recursion depth exceeded ({state.max_depth})")
    ...
    if isinstance(expr, str):
        if expr.startswith('"') and expr.endswith('"'):
            return expr[1:-1]  # strip quotes, return string value
    ...
    if head == "let":
        ...
        for b in bindings:
            ...
            if not isinstance(bname, str):
                raise LispyRuntimeError("let binding name must be symbol")
    ...
    if isinstance(func, Closure):
        if len(args) != len(func.params):
            raise LispyRuntimeError(
                f"expected {len(func.params)} args, got {len(args)}")
        child = Env(bindings=dict(zip(func.params, args)),
                    parent=func.env)
...
def _make_stdlib() -> dict:
    ...
    def _div(a, b):
        if b == 0:
            raise LispyRuntimeError("division by zero")
        return a / b

    def _mod(a, b):
        if b == 0:
            raise LispyRuntimeError("modulo by zero")
        return a % b
    ...
    def _sqrt(x):
        if x < 0:
            raise LispyRuntimeError("sqrt of negative")
        return math.sqrt(x)
    ...
    def _list_fn(*args):
        ...
        if len(result) > MAX_LIST_SIZE:
            raise LispyBudgetExceeded(
                f"list size exceeds {MAX_LIST_SIZE}")
        return result
    ...
    def _string_append(*args):
        ...
        if len(result) > MAX_STRING_LEN:
            raise LispyBudgetExceeded(
                f"string exceeds {MAX_STRING_LEN}")
        return result

    return {
        "+": _add, "-": _sub, "*": _mul, "/": _div, "%": _mod,
        ">": _gt, "<": _lt, ">=": _gte, "<=": _lte, "=": _eq,
        "!=": _neq,
...
def make_env(extra: dict = None) -> Env:
...
def run(source: str, extra_bindings: dict = None,
        max_steps: int = MAX_STEPS,
        max_depth: int = MAX_DEPTH) -> Any:
    """Parse and evaluate a LisPy program. Returns the last value."""
    exprs = parse_all(source)
    total_nodes = sum(_count_nodes(e) for e in exprs)
    if total_nodes > MAX_AST_NODES:
        raise LispyBudgetExceeded(
            f"AST size {total_nodes} exceeds limit {MAX_AST_NODES}")
