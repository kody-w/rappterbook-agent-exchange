...
# ---------------------------------------------------------------------------
# Hanson's LMSR: cost(q) = b * ln(sum(exp(q_i / b)))
# Price of outcome i = exp(q_i / b) / sum(exp(q_j / b))
# This is the gold standard for automated market makers in prediction markets.

DEFAULT_LIQUIDITY = 100.0  # b parameter — higher = more liquidity, tighter spreads
...
def lmsr_trade_cost(
    ...
    Returns the amount the trader must pay (positive = cost, negative = refund).
    """
...
class Market:
    ...
    quantities: list[float]
    liquidity: float
    resolution: int | None = None  # index of winning outcome, or None if unresolved
    ...
    def buy(self, outcome_idx: int, shares: float, trader_id: str, sol: int = 0) -> Trade:
        """Execute a buy order. Returns the trade."""
    ...
    def pnl(self, trader_id: str) -> float:
        """Compute P&L for a trader after resolution.

        Winning shares pay 1.0 each. Losing shares pay 0.0.
        P&L = (winning_shares * 1.0) - total_cost_paid.
        """
    ...
    def snapshot(self) -> dict:
        ...
        return {
            ...
            "winning_outcome": self.outcomes[self.resolution] if self.resolution is not None else None,
        }


# ---------------------------------------------------------------------------
# Market factory — create markets from colony data
# ---------------------------------------------------------------------------
...
def create_colony_markets(colony_names: list[str], strategies: list[str]) -> list[Market]:
    ...
    # 2. Population threshold markets
    ...
    # 6. Tech race — first to unlock 4+ techs
    markets.append(Market(
...
TRADER_PROFILES = [
    {"id": "oracle", "style": "informed", "bankroll": 500.0, "desc": "Trades on Monte Carlo data"},
    {"id": "bull", "style": "optimist", "bankroll": 300.0, "desc": "Bets on survival and growth"},
    {"id": "bear", "style": "pessimist", "bankroll": 300.0, "desc": "Bets on failure and collapse"},
    {"id": "contrarian", "style": "contrarian", "bankroll": 200.0, "desc": "Fades the crowd"},
    {"id": "random-walk", "style": "noise", "bankroll": 150.0, "desc": "Random noise trader"},
    {"id": "techie", "style": "tech_bull", "bankroll": 250.0, "desc": "Bets on tech-heavy colonies"},
]
...
def _deterministic_rng(trader_id: str, market_id: str) -> random.Random:
    """Deterministic RNG per trader-market pair."""
...
def simulate_trading(
    ...
    mc_stats: Monte Carlo statistics (if available) for informed trading.
    """
    traders = [
    ...
    for round_num in range(rounds):
        for market in markets:
            ...
            for trader in traders:
                rng = _deterministic_rng(trader.trader_id, f"{market.market_id}:{round_num}")

                shares, outcome_idx = _decide_trade(
                    trader, market, prices, mc_stats, rng, round_num,
                )
                if shares <= 0:
                    continue

                cost = lmsr_trade_cost(market.quantities, outcome_idx, shares, market.liquidity)
...
def _decide_trade(
    ...
    """Decide what to trade. Returns (shares, outcome_idx)."""
...
def _informed_trade(market: Market, mc_stats: dict, rng: random.Random) -> tuple[float, int]:
    """Oracle trades on Monte Carlo survival/growth data."""
    ...
    if mid == "fastest-grower" and growth_stats:
        best_idx = max(range(len(growth_stats)), key=lambda j: growth_stats[j].get("mean", 0))
        return (rng.randint(5, 15), best_idx)

    if mid == "largest-pop":
        final_pops = mc_stats.get("final_pop_stats", [])
        if final_pops:
            best_idx = max(range(len(final_pops)), key=lambda j: final_pops[j].get("mean", 0))
            return (rng.randint(5, 15), best_idx)
...
def _optimist_trade(market: Market, prices: list[float], rng: random.Random) -> tuple[float, int]:
    """Bull buys 'Yes' on survival, growth markets."""
...
def _pessimist_trade(market: Market, prices: list[float], rng: random.Random) -> tuple[float, int]:
    """Bear bets on failure."""
...
def _contrarian_trade(market: Market, prices: list[float], rng: random.Random) -> tuple[float, int]:
    """Fades the most popular outcome."""
    cheapest_idx = min(range(len(prices)), key=lambda i: prices[i])
    return (rng.randint(3, 8), cheapest_idx)


def _tech_bull_trade(market: Market, prices: list[float], rng: random.Random) -> tuple[float, int]:
    """Bets on tech-related markets and aggressive colonies."""
    if "tech" in market.question.lower():
        return (rng.randint(5, 12), 2)  # Aggressive colony (Red Frontier, idx 2)
    if "exceed" in market.question.lower() and "200" in market.question:
        return (rng.randint(3, 8), 0)
    return (rng.randint(1, 4), rng.randint(0, len(market.outcomes) - 1))


def _noise_trade(market: Market, prices: list[float], rng: random.Random) -> tuple[float, int]:
    """Random walk."""
...
def resolve_markets(markets: list[Market], sim_results: dict) -> dict:
    ...
    summary_colonies = sim_results.get("summary", {}).get("colonies", [])
    ...
    for market in markets:
        ...
        if mid.startswith("survival-"):
            ...
            if colony_data:
                tech_leader = max(colony_data.values(), key=lambda x: x["techs"])
                idx = tech_leader["idx"]
                market.resolve(idx)
                resolutions[mid] = tech_leader["name"]
...
def score_traders(markets: list[Market], traders: list[Trader]) -> list[dict]:
    """Compute final P&L for each trader across all markets."""
    scores = []
    for trader in traders:
        total_pnl = 0.0
        market_pnls = {}
...
def run_prediction_market(
    ...
    Returns serializable results dict.
    """
    colonies = sim_results.get("colonies", [])
    ...
    if verbose:
        ...
        for m in markets:
            prices = m.prices()
            price_str = " | ".join(f"{o}: {p:.1%}" for o, p in zip(m.outcomes, prices))
            print(f"    [{m.market_id}] {m.question}")
            print(f"      Opening: {price_str}")

    # Trade
    if verbose:
        print(f"\n  Simulating {trading_rounds} rounds of trading with {len(TRADER_PROFILES)} traders...")
    ...
    if verbose:
        print("\n  TRADER LEADERBOARD:")
        print(f"    {'Trader':<15} {'Style':<12} {'Spent':>8} {'P&L':>8} {'ROI':>8}")
        print(f"    {'-'*15} {'-'*12} {'-'*8} {'-'*8} {'-'*8}")
        for s in scores:
            print(f"    {s['trader_id']:<15} {s['style']:<12} "
                  f"${s['spent']:>7.2f} ${s['total_pnl']:>7.2f} {s['roi_pct']:>6.1f}%")
    ...
    if verbose:
        ...
        if best:
            print(f"  Best trader: {best['trader_id']} ({best['style']}) "
                  f"with ${best['total_pnl']:.2f} P&L ({best['roi_pct']:.1f}% ROI)")
        print()
...
def main() -> None:
    ...
    parser.add_argument("--live", action="store_true", help="Run sim first, then markets")
    parser.add_argument("--sols", type=int, default=365)
    parser.add_argument("--seeds", type=int, default=10, help="Monte Carlo seeds")
    ...
    if args.live:
        ...
        for col in sim_results["colonies"]:
            ...
            techs = col.get("tech", {}).get("unlocked_count", 0)
            print(f"    {name}: {start} → {end} ({growth:+.1f}%) [{techs} techs]")
        ...
        if not mars_path.exists():
            print(f"ERROR: {mars_path} not found. Run with --live or run main.py first.")
            sys.exit(1)
