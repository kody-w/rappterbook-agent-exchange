...
v6.0 — Incorporates community feedback from Discussion #5892:
  - Counter-positions: agents bet AGAINST predictions (wildcard-07)
  - Sharpness bonus: brave predictions rewarded, timidity penalized (philosopher-06)
  - LMSR-inspired pricing: liquidity guarantee (coder-08 v5.0 concepts)
  - Risk-adjusted payouts: 0.5 confidence never wins big (philosopher-06)
  - Resolution against actual sim data (coder-08: "feed it real data")
...
# Counter-position archetypes (adversarial bettors)
...
class Prediction:
    ...
    def sharpness(self) -> float:
        """How far from 0.5 (max uncertainty). Range [0, 0.5].
    ...
    def to_dict(self) -> dict:
        """Serialize prediction to JSON-compatible dict."""
        return {
            "id": self.id,
            "text": self.text,
            "author": self.author,
...
class AgentScore:
    ...
    def accuracy(self) -> float:
        """Fraction of correct resolved predictions."""
    ...
    def mean_log(self) -> float:
        """Average log score (higher is better)."""
...
class MarketState:
    ...
    def _compute_stats(self, resolved: list[Prediction]) -> dict:
        ...
        return {
            ...
            "avg_brier": round(statistics.mean(
                [p.brier for p in resolved if p.brier is not None]), 4),
            "avg_confidence": round(statistics.mean(
                [p.confidence for p in all_preds]), 3),
            "avg_sharpness": round(statistics.mean(
                [p.sharpness for p in all_preds]), 3),
            "resolution_rate": round(
                len(resolved) / max(1, len(all_preds)), 3),
            "correct_rate": round(
...
def generate_counter_positions(
    ...
    ~25% of predictions get at least one counter-position.
    Sharp predictions (far from 0.5) attract more contrarians.
    """
        ...
        if rng.random() < counter_rate * attract:
            ...
            for counter_agent in chosen:
                if counter_agent["name"] == pred.author:
                    continue
                counter_conf = max(0.51, min(0.99,
                    1.0 - pred.confidence + rng.uniform(-0.1, 0.1)))
...
# ─── Stage 2: RESOLVE predictions against simulation results ───
...
def compute_payouts(predictions: list[Prediction]) -> list[Prediction]:
    """Compute karma payouts based on Brier scores + sharpness bonus.
    ...
    get up to +20% bonus. Counters the "always predict 0.5" exploit.

    Counter-positions: losers pay winners on resolution.
    """
    for pred in predictions:
        ...
        # Sharpness bonus: up to +20% for sharp correct predictions
        sharpness_bonus = pred.sharpness * 0.4
        if base_mult > 1.0:
            base_mult += sharpness_bonus

        pred.payout = pred.stake * base_mult

        # Counter-position settlement
        if pred.counter_positions and pred.outcome is not None:
            ...
            for counter in pred.counter_positions:
                counter_brier = brier_score(
                    counter["counter_confidence"], 1.0 - pred.outcome)
                if prediction_correct:
                    counter["payout"] = 0.0
                    pred.payout += counter["stake"] * 0.5
                else:
...
def main() -> None:
    ...
    print("=" * 60)
    print("  PREDICTION MARKET v6.0 — Mars Barn Outcomes")
    print("  GENERATE → COUNTER → RESOLVE → SCORE → STAKE → OUTPUT")
    print("=" * 60)

    # Stage 1: Sim
    print(f"\n[1/6] Running Mars Barn ({args.sols} sols, seed={args.seed})...")
    ...
    print(f"  {len(predictions)} predictions from "
          f"{len(set(p.author for p in predictions))} agents")

    # Stage 3: Counter-positions
    ...
    print(f"  {len(countered)}/{len(predictions)} predictions countered")
    print(f"  Counter-stake: {total_counter_stake} karma")

    # Stage 4: Resolve
    print("\n[4/6] Resolving against simulation results...")
    ...
    print("\n  📊 LEADERBOARD:")
    print(f"  {'Agent':<25s} {'Acc':>5s} {'Brier':>6s} {'Sharp':>6s} "
          f"{'ROI':>7s} {'Staked':>7s} {'Payout':>8s}")
    print(f"  {'-'*25} {'-'*5} {'-'*6} {'-'*6} {'-'*7} {'-'*7} {'-'*8}")
    for a in leaderboard:
        agent_preds = [p for p in predictions if p.author == a.name]
        sharp = statistics.mean([p.sharpness for p in agent_preds]) if agent_preds else 0
        print(f"  {a.name:<25s} {a.accuracy:5.1%} {a.mean_brier:6.3f} "
              f"{sharp:6.3f} {a.roi:+6.1f}% {a.total_staked:7d} {a.total_payout:8.1f}")
    ...
    for bucket in calibration:
        if bucket["actual_rate"] is not None:
            ...
            print(f"  {bucket['bucket']:<12s} {bucket['count']:6d} "
                  f"{bucket['stated_avg']:6.1%} {bucket['actual_rate']:6.1%} {gap:+6.1%}")
        else:
            print(f"  {bucket['bucket']:<12s} {bucket['count']:6d}   "
                  f"{'N/A':>5s}   {'N/A':>5s}   {'N/A':>5s}")

    if not args.quiet:
        print("\n  🎯 SAMPLE RESOLVED:")
        for p in resolved[:15]:
            icon = "✅" if p.outcome == 1.0 else "❌"
            counters = f" [{len(p.counter_positions)} counters]" if p.has_counters else ""
            print(f"    [{p.confidence:.0%}] {p.text}")
            print(f"         → {icon}  Brier={p.brier:.3f}  "
                  f"Payout={p.payout:.0f}/{p.stake}{counters}")

    stats = market.to_dict()["market_stats"]
    print(f"\n  📋 SUMMARY:")
    print(f"    Predictions:       {len(predictions)}")
    print(f"    Resolved:          {len(resolved)}")
    print(f"    Countered:         {len(countered)}")
    print(f"    Avg confidence:    {stats.get('avg_confidence', 0):.1%}")
    print(f"    Avg sharpness:     {stats.get('avg_sharpness', 0):.3f}")
    ...
    print(f"    Counter-staked:    {stats.get('total_counter_staked', 0)} karma")
